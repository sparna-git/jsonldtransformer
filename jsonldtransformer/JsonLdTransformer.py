...
def apply_rules(shapes_rules: Dict, data_graph: GraphLike, iterate=False) -> Graph:
    ...
    for shape, rules in sorted_shapes_rules:
        ...
        while True:
            if iterate_limit < 1:
                raise RuntimeError("SHACL Shape Rule iteration exceeded iteration limit of 100.")
            iterate_limit -= 1
            this_modified = 0
            collect_graph = Graph()
...
def apply_sparql_rule(rule, data_graph: GraphLike) -> Graph:
	...
	while True:
		if iterate_limit < 1:
			raise RuntimeError("Local SPARQLRule iteration exceeded iteration limit of 100.")
		iterate_limit -= 1
		added = 0
		...
		for a in applicable_nodes:
			for c in rule._constructs:
				...
				if results.type != "CONSTRUCT":
					raise RuntimeError("Query executed by a SHACL SPARQLRule must be CONSTRUCT query.")
				this_added = False
				result_graph = results.graph
				if result_graph is None:
					raise RuntimeError("Query executed by a SHACL SPARQLRule did not return a Graph.")
